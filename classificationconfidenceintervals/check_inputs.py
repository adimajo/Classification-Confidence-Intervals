...
def check_init_params(  # noqa: C901
    ...
    Args:
        ...
        population_flagged_count (int): Number of datapoints in population flagged as positives by algorithm.
    ...
    """
    # sample labels
    if set(sample_labels) not in [{True, False}, {0, 1}, {-1, 1}]:
        raise ValueError("sample_labels must be boolean or be binary in [0,1] or [-1,1].")

    # sample predictions
    if set(sample_predictions) not in [{True, False}, {0, 1}, {-1, 1}]:
    ...
    if not isinstance(population_size, int) or not population_size > 0:
        raise ValueError(f"population_size {population_size} must be a positive integer.")
    ...
    # exact_precision
    if exact_precision is not None and (not isinstance(exact_precision, float) or not 0 <= exact_precision <= 1):
        raise ValueError(
